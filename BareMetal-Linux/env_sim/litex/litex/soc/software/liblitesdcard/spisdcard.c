...
static int spisdcard_write_block(const uint8_t *buf, uint8_t token)
{
    uint8_t resp;

    // Ensure card is ready (wait for 0xFF)
    while (spi_xfer(0xFF) != 0xFF);

    // Send start token (0xFE normal / 0xFC multi / 0xFD stop)
    spi_xfer(token);
    ...
    if (token != 0xFD) {
        ...
        // Read response token, must be 0bxxx00101
        do {
            resp = spi_xfer(0xFF);
        } while (resp == 0x00);
    ...
    // Wait until card finishes internal write (returns 0xFF)
    while (spi_xfer(0xFF) != 0xFF);

    return 1; // SUCCESS
}

DRESULT spisd_disk_write(BYTE drv, const BYTE *buf, LBA_t block, UINT count)
{
    uint8_t cmd;
    ...
    // Write each block
    while (count > 0) {
        uint8_t token = (cmd == CMD25) ? 0xFC : 0xFE;  // CMD25 uses 0xFC start token

        if (!spisdcard_write_block(buf, token))
            break;
    ...
    if (cmd == CMD25) {
        ...
        spisdcard_write_block(NULL, 0xFD);
    }
